            ...
            for d in [['int', 25_000_000], ['rng4', 25_000_000], ['data/urls-short', 3766956], ['data/wiki', 13816679]]:
                for var_size_b in range(8, 16):
                    ...
                    if var_size>={'data/urls-short':10,'data/wiki':9,'int':0,'rng4':0}[d[0]]:
                        for config in ["baseline", "dense1", "dense2", "dense3", "hash", "heads", "hints","adapt", "inner", "prefix"]:
                            print_work(d, config, run_id, workload, var_size,4096)
                        if var_size!=4096:
                            for config in ["baseline", "heads", "hints", "inner", "prefix"]:
                                print_work(d, config, run_id, workload, var_size,4096)
                            print_work(d, 'hints', run_id, workload,  4096,var_size)
# %%
