...
{
    // this will be compiled into one vpcmpeqb instruction (AVX2.)
    // that is hella fast.
    auto m = reinterpret_cast<__m256i>(*a == *b);

    // compact into bitvec. also intrinsics for fastness.
    return static_cast<HashSimdBitMask>(_mm256_movemask_epi8(m));
}

int HashNode::findIndexSimd(uint8_t* key, unsigned keyLength, uint8_t hash)
{
   auto* aligned_this = std::assume_aligned<alignof(HashSimdVecByte)>(this);
   (void)aligned_this;
