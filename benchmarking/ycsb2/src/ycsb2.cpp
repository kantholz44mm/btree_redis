...
void runYcsbC(BTreeCppPerfEvent e, std::vector<std::string>& data, unsigned keyCount, unsigned payloadSize, unsigned opCount, double zipfParameter, bool dryRun)
{
   if (keyCount <= data.size() && keySizeAcceptable(payloadSize, data)) {
      if (!dryRun)
         std::random_shuffle(data.begin(), data.end());
...
void runSortedInsert(BTreeCppPerfEvent e, std::vector<std::string>& data, unsigned keyCount, unsigned payloadSize, bool dryRun, bool doSort = true)
{
   if (keyCount <= data.size() && keySizeAcceptable(payloadSize, data)) {
      data.resize(keyCount);
      if (!dryRun && doSort) {
